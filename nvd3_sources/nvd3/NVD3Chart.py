...
class NVD3Chart:
	...
	**Attributes**:
	...
	def __init__(self, **kwargs):
		...
		# added by ammar
		self.reduceXTicks = kwargs.get("reduceXTicks", True)
		self.rotateLabels = kwargs.get("rotateLabels", 0)
		self.margin = kwargs.get("margin", {'top': 30, 'right': 60, 'bottom': 50, 'left': 70})
		self.stacked = kwargs.get("stacked", False)
		self.showControls = kwargs.get("showControls", True)
	...
	def buildjschart(self):
		...
		# added by ammar
		self.jschart += stab(2) + 'chart.margin(%s);\n' % json.dumps(self.margin)

		if self.model == 'multiBarChart':
			self.jschart += stab(2) + 'chart.showControls(%s);\n' % json.dumps(self.showControls)
			self.jschart += stab(2) + 'chart.reduceXTicks(%s);\n' % str(self.reduceXTicks).lower()
			self.jschart += stab(2) + 'chart.rotateLabels(%s);\n' % self.rotateLabels
			self.jschart += stab(2) + 'chart.multibar.stacked(%s);\n' % json.dumps(self.stacked)
